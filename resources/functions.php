...
function upload_image($name, &$cove_id)
{
    global $pdo;
    //**------  Handling  Image -------*/
    if (!empty($_FILES[$name]['name'])) {
        ...
        if ($cover_pic_type == "image/jpg" || $cover_pic_type = "image/jpeg" || $cover_pic_type = "image/png" || $cover_pic_type = "image/gif") // check file extension
        {
            ...
            if ($cover_pic_size < 2000000) { //check if file size below 2mb
                ...
                set_message('error', 'Your file To large PLease Upload 2mb size');
            }
...
function create_url_slug($string){
    $slug_fr=preg_replace('/[^A-Za-z0-9-]+/', '-', $string);
    return $slug_fr;
}
        ...
        if(isset($_POST['submit'])) {
            ...
            if($res){
                if($current_pass == $res->password_hash){
                    ...
                    if($updatepass){
                        set_message('success', 'Mot de passe mis à jour avec succès');
                        redirect('index');
                    }else{
                        set_message('error', 'query failed try later');
                    }
                }else {
                    set_message('error', 'Mot de passe actuel incorrect');
                }
            } else {
                set_message('error', 'Administrateur nest pas dans notre système');
            }
            }
...
require_once('component/eventsComponent.php');
require_once('component/blogComponent.php');
require_once('component/categoryComponent.php');
require_once('component/contactComponent.php');
require_once('component/newsletterComponent.php');
require_once('component/studentsComponent.php');
// require_once('component/infosComponent.php');
